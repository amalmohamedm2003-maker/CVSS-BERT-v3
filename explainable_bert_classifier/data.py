...
def split_dataset(dataset, labels, encoded_labels, tokenizer, val_proportion=0.2, shuffle=True):
    """
    Split dataset, labels, and encoded_labels and return two dataset objects: one for training and another for validation/testing.
    ...
    train_encodings = tokenizer(X_train["Description"].tolist(), truncation=True, padding=True, max_length=128)
    val_encodings = tokenizer(X_val["Description"].tolist(), truncation=True, padding=True, max_length=128)
...
class CVEDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, X, encodings, labels, encoded_labels):
        """
        Args:
            X: CVE vulnerability description dataset. Must contain two columns: "CVE_ID" and "Description".
            encodings: tokenized representation of the descriptions contained in X
            labels: the text form labels of the descriptions contained in X
            encoded_labels: corresponding encoded labels
        """
        self.cve_id = X["CVE_ID"].tolist()
        self.texts = X["Description"].tolist()
